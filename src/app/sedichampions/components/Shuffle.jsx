  ...
  useGSAP(
    () => {
      ...
      const create = () => {
        ...
        if (triggerOnHover) {
          // No animar al cargar: dejar el texto ya "armado" en su estado final
          // y esperar a que pase el mouse para hacer el shuffle (cada vez).
          cleanupToStill();
          if (colorTo) {
            const reals = wrappersRef.current
              .map(w => w.querySelector('[data-orig="1"]'))
              .filter(Boolean);
            gsap.set(reals, { color: colorTo });
          }
          onShuffleComplete?.();
        } else {
          play();
